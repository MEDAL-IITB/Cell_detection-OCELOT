...
#             elif len(gt_cls) == 0:
#                 distance = np.zeros([0, len(pred_cls)])
#                 confidence = np.zeros([0, len(pred_cls)])
            else:
                #print(gt_cls.shape)
...
def main():
    ...
    algorithm_output_path = "/workspace/jay/DDP/Ocelot/ocelot23algo/evaluation/cellonlyseg_train.json"
    with open(algorithm_output_path, "r") as f:
        pred_json = json.load(f)["points"]
    
    # Path where GT is stored
    gt_path = "/workspace/jay/DDP/Ocelot/ocelot23algo/evaluation/cell_gt_traintrue.json"
    with open(gt_path, "r") as f:
        gt_json = json.load(f)["points"]
        num_images = 400#json.load(f)["num_images"]
